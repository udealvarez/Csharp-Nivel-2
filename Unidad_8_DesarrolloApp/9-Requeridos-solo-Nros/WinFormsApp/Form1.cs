...
namespace WinFormsApp
{
    public partial class Form1 : Form
    {
        ...
        private void txtFiltro_TextChanged(object sender, EventArgs e) // ESTE FILTRO ES PARA CUANDO VAS ESCRIBIENDO EL NOMBRE Y SE VA FILTRANDO-> PIKA -> TE TRAE PIKACHU
        {
        ...
        {
            if (cbCampo.SelectedIndex < 0) // si es mayor o igual a cero, sig que tiene algo seleccionado en el combo
            {                              // si es menor a cero quiere decir que no hay nada seleccionado en el combo
            ...
            {
                if (string.IsNullOrEmpty(txtFiltroAvanzado.Text))   // valida que el campo no este vacio
                {
        ...
        private bool soloNumeros(string cadena)
        {
            ...
            {
                if (!(char.IsNumber(caracter))) // si no es numero el caracter
                {
                    return false;
                }
            }
        ...
        private void btnFiltro_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (validarFiltro())
                {
                    return;     // este return cancela el evento pq tiene que ser true, en el caso que el return me de false
                }
